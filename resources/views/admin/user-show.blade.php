<x-admin-layout>
    ...
    <div x-data="{ activeTab: 'overview' }" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 overflow-hidden">
        ...
        <div class="border-b border-gray-200 dark:border-gray-700 overflow-x-auto scroller-hide">
            <nav class="flex min-w-max">
                ...
                <button @click="activeTab = 'security'"
                    :class="{ 'border-indigo-500 text-indigo-600 dark:text-indigo-400': activeTab === 'security', 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400': activeTab !== 'security' }"
                    class="px-3 sm:px-6 py-3 sm:py-4 text-xs sm:text-sm font-medium border-b-2 whitespace-nowrap transition-colors">
                    Security
                </button>
            </nav>
        ...
        <div class="p-4 sm:p-6">
            ...
            <div x-show="activeTab === 'security'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-y-2" x-transition:enter-end="opacity-100 translate-y-0">
                <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 p-4 sm:p-6">
                    <h3 class="text-base sm:text-lg font-bold text-gray-900 dark:text-white mb-4 sm:mb-6">
                        <i class="fas fa-shield-alt mr-2 text-indigo-500"></i>Security Settings
                    </h3>
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 sm:p-5 border border-gray-200 dark:border-gray-600">
                            <div class="flex items-start gap-4">
                                <div class="w-12 h-12 bg-orange-100 dark:bg-orange-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-key text-xl text-orange-600 dark:text-orange-400"></i>
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-bold text-gray-900 dark:text-white text-sm sm:text-base mb-1">Reset Password</h4>
                                    <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-4">
                                        Reset user's password to the default: <code class="px-2 py-0.5 bg-gray-200 dark:bg-gray-600 rounded text-xs font-mono">password123</code>
                                    </p>
                                    <button onclick="document.getElementById('resetPasswordModal').classList.remove('hidden')"
                                        class="px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-xs sm:text-sm font-semibold transition-colors shadow-sm">
                        ...
                        <div class="bg-gray-50 dark:bg-gray-700/50 rounded-xl p-4 sm:p-5 border border-gray-200 dark:border-gray-600">
                            <div class="flex items-start gap-4">
                                <div class="w-12 h-12 bg-blue-100 dark:bg-blue-900/30 rounded-xl flex items-center justify-center flex-shrink-0">
                                    <i class="fas fa-user-shield text-xl text-blue-600 dark:text-blue-400"></i>
                                </div>
                                <div class="flex-1">
                                    <h4 class="font-bold text-gray-900 dark:text-white text-sm sm:text-base mb-1">Account Status</h4>
                                    <p class="text-xs sm:text-sm text-gray-500 dark:text-gray-400 mb-2">
                                        Current Status:
                                        @if($user->checkBlacklist())
                                        <span class="px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 rounded text-xs font-semibold">Blacklisted</span>
                                        @else
                                        <span class="px-2 py-0.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400 rounded text-xs font-semibold">Active</span>
                                        @endif
                                    </p>
                                    <p class="text-xs text-gray-400 dark:text-gray-500">
                                        Created: {{ $user->created_at->format('d M Y') }}<br>
    ...
    <div id="resetPasswordModal" class="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center hidden z-50 p-4">
        <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl w-full max-w-sm p-6">
            <div class="text-center mb-4">
                <div class="w-16 h-16 bg-orange-100 dark:bg-orange-900/30 rounded-full flex items-center justify-center mx-auto mb-4">
                    <i class="fas fa-key text-2xl text-orange-600 dark:text-orange-400"></i>
                </div>
                <h3 class="text-lg font-bold text-gray-900 dark:text-white mb-2">Reset Password</h3>
                <p class="text-gray-500 dark:text-gray-400 text-sm">
                    Are you sure you want to reset the password for <span class="font-bold text-gray-900 dark:text-white">{{ $user->name }}</span>?
                </p>
                <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">
                    New password will be: <code class="px-2 py-0.5 bg-gray-200 dark:bg-gray-600 rounded font-mono">password123</code>
                </p>
            </div>
            <div class="flex gap-3">
                <button onclick="document.getElementById('resetPasswordModal').classList.add('hidden')" class="flex-1 px-4 py-2 bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 rounded-xl font-medium hover:bg-gray-200 dark:hover:bg-gray-600 transition-colors">Cancel</button>
                <form action="{{ route('admin.users.resetPassword', $user->id) }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full px-4 py-2 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-medium shadow-lg transition-colors">
